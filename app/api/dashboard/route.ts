  ...
  try {
    ...
    if (gameIds.length > 0) {
      ...
      const [
        ...
        // Average grade (user team only)
        db
          .select({
            avg: sql<number>`COALESCE(AVG(${playerAnalysis.overallGrade}), 0)`,
          })
          .from(playerAnalysis)
          .innerJoin(
            detectedPlayers,
    ...
    if (userTeam.length > 0 && userTeam[0].teams.sportsTeamId) {
      ...
      const [teamInfoResult, rosterData] = await Promise.all([
        db.select().from(sportsTeams).where(eq(sportsTeams.id, sportsTeamId)),
        db
          .select()
    ...
    if (latestReadyGame) {
      ...
      parallelQueries.push(
        db
          ...
          .innerJoin(
            detectedPlayers,
            eq(detectedPlayers.id, keyMoments.detectedPlayerId),
          )
    ...
    if (resultMap.has("moments") && latestReadyGame) {
      ...
      recentMoments = moments.map((m: any) => ({
        ...
        timestamp: m.timestampSeconds
          ? formatTimestamp(parseFloat(m.timestampSeconds))
          : null,
        timestampSeconds: m.timestampSeconds
          ? parseFloat(m.timestampSeconds)
          : null,
    ...
    if (resultMap.has("insights")) {
      ...
      if (insightsResult[0]?.geminiAnalysis) {
        ...
        if (analysis.coachingInsights?.insights) {
          ...
          coachingInsights = analysis.teamScouting.homeTeam.tendencies.slice(
            0,
            3,
          );
    ...
    if (resultMap.has("playerGrades")) {
      ...
      for (const [jerseyNumber, grades] of playerMap) {
        ...
        const latestGrade = parseFloat(
          grades[0].overallGrade?.toString() || "0",
        );
        ...
        // Calculate trend (compare latest game to previous game)
        let trend: "up" | "down" | "stable" = "stable";
        if (grades.length >= 2) {
          // grades are sorted by date DESC, so index 0 is most recent
          const latestGradeVal = parseFloat(
            grades[0].overallGrade?.toString() || "0",
          );
          const previousGradeVal = parseFloat(
            grades[1].overallGrade?.toString() || "0",
          );

          const diff = latestGradeVal - previousGradeVal;
          if (diff >= 3) trend = "up";
        ...
        playerDevelopment.push({
          ...
          avgGrade:
            grades.reduce(
              (sum: number, g: any) =>
                sum + parseFloat(g.overallGrade?.toString() || "0"),
              0,
            ) / grades.length,
        });
      }
    ...
    if (resultMap.has("playersWithMetrics")) {
      ...
      const allStats = Array.from(statsMap.entries()).map(
        ([jersey, stats]) => ({
          jerseyNumber: jersey,
          name: stats.name,
        ...
        if (Array.isArray(areas)) {
          for (const area of areas) {
            ...
            if (areaText) {
              ...
              focusAreaCounts.set(
                normalized,
                (focusAreaCounts.get(normalized) || 0) + 1,
              );
            }
          }
        }
    ...
    if (resultMap.has("gameGrades")) {
      ...
      seasonProgression = gameGrades
        .filter(
          (g: any) =>
            g.avgGrade !== null &&
            parseFloat(g.avgGrade?.toString() || "0") > 0,
        )
        .map((g: any) => ({
          gameId: g.gameId,
          date: g.gameDate?.toISOString(),
    ...
    const positionStats: Record<
      string,
      { count: number; totalGrade: number; players: string[] }
    > = {};
    ...
    const positionBreakdown = Object.entries(positionStats)
      .map(([position, stats]) => ({
        position,
