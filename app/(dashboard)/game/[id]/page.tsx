...
function getGradeColor(grade: string): string {
  ...
  if (g === "B" || g === "B+" || grade === "above-average")
...
function parseBoxScore(boxScore: string | null): {
  ...
  const quarterSection = boxScore.match(
    /Score By Quarter[\s\S]*?Final([\s\S]*?)(?:\n\n|Game Notes|$)/,
  );
  if (quarterSection) {
    ...
    const allMatches = scoresLine.matchAll(
      /([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)(\d{1,2})(\d{2})(\d{2})(\d{2})(\d{2,3})/g,
    );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div
          className={`sticky top-0 z-10 p-6 border-b ${player.team?.toLowerCase() === "home" ? "bg-blue-600" : "bg-orange-500"}`}
        >
          <button
            onClick={onClose}
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="text-white">
              ...
              <div className="flex items-center gap-2 mt-1 text-white/80">
                <span>
                  {player.position || player.estimatedPosition || "Player"}
                </span>
                <span>•</span>
                <span>
                  {teamName ||
                    (player.team?.toLowerCase() === "home"
                      ? "Home Team"
                      : "Away Team")}
                </span>
              </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-3 gap-3">
            {player.basketballIQ && (
              <div className="bg-indigo-50 dark:bg-indigo-900/30 rounded-xl p-4 text-center">
                <div className="text-lg font-bold text-indigo-900 dark:text-indigo-300 capitalize">
                  {player.basketballIQ}
                </div>
            ...
            {player.defensiveRating && (
              <div className="bg-red-50 dark:bg-red-900/30 rounded-xl p-4 text-center">
                <div className="text-lg font-bold text-red-900 dark:text-red-300 capitalize">
                  {player.defensiveRating}
                </div>
            ...
            {player.motor && (
              <div className="bg-green-50 dark:bg-green-900/30 rounded-xl p-4 text-center">
                <div className="text-lg font-bold text-green-900 dark:text-green-300 capitalize">
                  {player.motor}
                </div>
            ...
            <div className="space-y-3">
              {player.preferredHand && (
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium text-blue-900 dark:text-blue-200 capitalize">
                    {player.preferredHand}
                  </span>
                </div>
              )}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {player.primaryMoves.map((move: string, i: number) => (
                      <span
                        key={i}
                        className="bg-blue-100 dark:bg-blue-800/50 text-blue-800 dark:text-blue-200 px-3 py-1 rounded-full text-sm"
                      >
                        {move}
                      </span>
                    ))}
          ...
          {player.howToAttack && (
            <div className="bg-green-100 dark:bg-green-900/30 rounded-xl p-4">
              ...
              <p className="text-green-900 dark:text-green-400">
                {player.howToAttack}
              </p>
            </div>
          )}
...
function PlayerCard({
  ...
  geminiPlayerData?: any;
}) {
  const analysis = player.analysis;
  const grade = analysis?.overallGrade
    ? parseFloat(analysis.overallGrade)
    : null;
  const [flagged, setFlagged] = useState(false);

  // Get box score from either playerAnalysis.metrics or geminiPlayerData
  const boxScore =
    analysis?.metrics?.boxScore || geminiPlayerData?.boxScore || {};
  const hasStats =
    boxScore.points != null ||
    boxScore.rebounds != null ||
    boxScore.assists != null;
        ...
        <div className="flex items-center justify-between">
          ...
          {grade !== null && (
            <div className="text-right mr-6">
              <div
                className={`text-2xl font-bold ${grade >= 80 ? "text-green-600" : grade >= 60 ? "text-yellow-600" : "text-red-600"}`}
              >
                {grade.toFixed(0)}
              </div>
        ...
        {hasStats && (
          <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
            <div className="grid grid-cols-5 gap-2 text-center">
              ...
              <div>
                <div className="text-lg font-bold text-gray-900 dark:text-white">
                  {boxScore.totalRebounds ?? boxScore.rebounds ?? "-"}
                </div>
                <div className="text-xs text-gray-500 dark:text-gray-400">
                  REB
                </div>
            ...
            {/* Shooting Splits */}
            {(boxScore.fieldGoalsMade != null ||
              boxScore.threePointersMade != null) && (
              <div className="mt-2 flex justify-center gap-4 text-xs text-gray-500 dark:text-gray-400">
                {boxScore.fieldGoalsMade != null &&
                  boxScore.fieldGoalsAttempted != null && (
                    <span>
                      FG: {boxScore.fieldGoalsMade}/
                      {boxScore.fieldGoalsAttempted}
                    </span>
                  )}
                {boxScore.threePointersMade != null &&
                  boxScore.threePointersAttempted != null && (
                    <span>
                      3P: {boxScore.threePointersMade}/
                      {boxScore.threePointersAttempted}
                    </span>
                  )}
                {boxScore.freeThrowsMade != null &&
                  boxScore.freeThrowsAttempted != null && (
                    <span>
                      FT: {boxScore.freeThrowsMade}/
          ...
          <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
            ...
            <div className="flex flex-wrap gap-1">
              {player.keyMoments.slice(0, 3).map((moment: any) => {
                const time = moment.timestampSeconds
                  ? parseFloat(moment.timestampSeconds)
                  : null;
                return (
                  <button
                    key={moment.id}
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          {team.isUserTeam && (
            <span className="text-xs bg-[#0f2d52] text-white px-2 py-0.5 rounded-full">
              Your Team
            </span>
          )}
        </div>
            ...
            {Object.entries(tendencies)
              ...
              .map(([key, value]) => (
                <li key={key} className="flex items-start gap-2 text-sm">
                  <span className="text-[#0f2d52] dark:text-blue-400 mt-0.5">
                    •
                  </span>
          ...
          <div className="grid grid-cols-2 gap-4">
            {analysis?.offensiveMetrics && (
              <div>
                ...
                <div className="space-y-1">
                  {Object.entries(analysis.offensiveMetrics)
                    .slice(0, 3)
                    .map(([key, value]) => (
            ...
            {analysis?.defensiveMetrics && (
              <div>
                ...
                <div className="space-y-1">
                  {Object.entries(analysis.defensiveMetrics)
                    .slice(0, 3)
                    .map(([key, value]) => (
                      <div key={key} className="flex justify-between text-sm">
                        <span className="text-gray-600 dark:text-gray-400 capitalize">
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <span className="text-sm text-gray-600 dark:text-gray-400">
                {player.estimatedPosition || player.position || "Player"}
              </span>
              <span
              ...
              >
                {teamName ||
                  (player.team?.toLowerCase() === "home" ? "Home" : "Away")}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        ...
        <div>
          ...
          {(player.offensiveTendencies?.primaryMoves?.length > 0 ||
            player.primaryMoves?.length > 0) && (
            <div className="mt-2">
              <span className="text-xs text-gray-400">Moves:</span>
              <div className="flex flex-wrap gap-1 mt-1">
                {(
                  player.offensiveTendencies?.primaryMoves ||
                  player.primaryMoves ||
                  []
                )
                  .slice(0, 3)
                  .map((move: string, i: number) => (
                    <span
                      key={i}
                      className="text-xs bg-blue-50 dark:bg-blue-900 text-blue-700 dark:text-blue-300 px-1.5 py-0.5 rounded"
                    >
                      {move}
                    </span>
                  ))}
...
function renderTendency(t: any): string {
  ...
  if (t && typeof t === "object") {
    ...
    if (t.action)
      return `${t.action}${t.frequency ? ` (${t.frequency})` : ""}${t.notes ? ` - ${t.notes}` : ""}`;
...
function TeamScoutingCard({
  ...
  return (
    <div
      className={`bg-white dark:bg-gray-800 rounded-xl border-2 p-6 ${borderClass}`}
    >
      {/* Header */}
      <div className="flex items-center gap-3 mb-6">
        <div
        ...
        <div>
          <h3 className="text-lg font-bold text-gray-900 dark:text-white">
            {teamName || (teamLabel === "home" ? "Home Team" : "Away Team")}
          </h3>
          {team.jerseyColor && (
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className={`p-4 rounded-lg ${bgClass} dark:bg-gray-700`}>
          ...
          <div className="font-bold text-gray-900 dark:text-white text-sm">
            {team.offensiveSystem || "Not analyzed"}
          </div>
          {team.transitionStyle && (
            <div className="text-xs text-gray-600 dark:text-gray-400 mt-1">
              Transition: {team.transitionStyle}
            </div>
          )}
          {team.spacing?.formation && (
            <div className="text-xs text-gray-600 dark:text-gray-400 mt-1">
              Spacing: {team.spacing.formation}
            </div>
          )}
        </div>
        <div className={`p-4 rounded-lg ${bgClass} dark:bg-gray-700`}>
          ...
          <div className="font-bold text-gray-900 dark:text-white text-sm">
            {team.defensiveSystem || "Not analyzed"}
          </div>
          {team.pnrCoverage && (
            <div className="text-xs text-gray-600 dark:text-gray-400 mt-1">
              PnR:{" "}
              {typeof team.pnrCoverage === "string"
                ? team.pnrCoverage
                : team.pnrCoverage.primary || "N/A"}
            </div>
          )}
        </div>
      </div>
      ...
      {/* Key Players */}
      {(team.keyOffensivePlayers?.length > 0 ||
        team.keyDefenders?.length > 0) && (
        <div className="mb-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            {team.keyDefenders?.slice(0, 3).map((p: any, i: number) => (
              <div
                key={`def-${i}`}
                className="bg-blue-50 dark:bg-blue-900/30 rounded-lg p-3 border border-blue-100 dark:border-blue-800"
              >
                <div className="flex items-center gap-2 mb-1">
      ...
      {(team.offensiveWeaknesses?.length > 0 ||
        ...
        <div className="bg-red-50 dark:bg-red-900/30 rounded-lg p-4 border border-red-100 dark:border-red-800 mb-4">
          ...
          <ul className="space-y-2">
            {(team.offensiveWeaknesses || team.weaknesses || [])
              .slice(0, 4)
              .map((w: any, i: number) => (
                <li
                  key={i}
...
function GeminiInsights({ game }: { game: any }) {
  ...
  const allPlayers =
    game?.detectedTeams?.flatMap((team: any) => team.players || []) || [];
    ...
    <div className="space-y-6">
        ...
        <div className="bg-gradient-to-r from-slate-50 to-slate-100 dark:from-gray-800 dark:to-gray-800 rounded-xl border border-slate-200 dark:border-gray-700 p-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
            <div className="bg-white dark:bg-gray-700 rounded-lg p-3 text-center border border-gray-100 dark:border-gray-600">
              <div className="text-2xl font-bold text-gray-900 dark:text-white">
                {game.geminiAnalysis?.playerScouting?.players?.length ||
                  allPlayers.length}
              </div>
              <div className="text-xs text-gray-500 dark:text-gray-400">
                Players Scouted
            ...
            {game.videoDurationSeconds && (
              <div className="bg-white dark:bg-gray-700 rounded-lg p-3 text-center border border-gray-100 dark:border-gray-600">
                <div className="text-2xl font-bold text-gray-900 dark:text-white">
                  {Math.floor(game.videoDurationSeconds / 60)}:
                  {(game.videoDurationSeconds % 60).toString().padStart(2, "0")}
                </div>
                <div className="text-xs text-gray-500 dark:text-gray-400">
                  Duration
                </div>
          ...
          <div className="flex flex-wrap gap-3 mb-4">
            {game.geminiAnalysis?.teamScouting ? (
              <>
                <div className="flex items-center gap-2 bg-white dark:bg-gray-700 rounded-lg px-4 py-2 border border-gray-100 dark:border-gray-600">
                  ...
                  <span className="text-sm text-gray-500 dark:text-gray-400">
                    {game.geminiAnalysis.playerScouting?.players?.filter(
                      (p: any) => p.team?.toLowerCase() === "home",
                    ).length || 0}{" "}
                    players
                  </span>
                </div>
              ...
              game.detectedTeams?.map((team: any) => (
                <div
                  key={team.id}
                  className="flex items-center gap-2 bg-white dark:bg-gray-700 rounded-lg px-4 py-2 border border-gray-100 dark:border-gray-600"
                >
                  {team.primaryJerseyColor && (
                    <div
              ...
              <table className="w-full text-center">
                <thead>
                  <tr className="text-xs text-gray-500 dark:text-gray-400 uppercase">
                    ...
                    <th className="py-1 px-3 w-16 bg-gray-100 dark:bg-gray-600 rounded">
                      Final
                    </th>
                  </tr>
                </thead>
                <tbody className="text-sm">
                  {boxScoreData.quarterScores.map((qs, i) => {
                    const isWinner =
                      qs.final ===
                      Math.max(
                        ...boxScoreData.quarterScores.map((q) => q.final),
                      );
                    return (
                      <tr key={i} className={isWinner ? "font-bold" : ""}>
                        ...
                        <td
                          className={`py-2 px-3 bg-gray-100 dark:bg-gray-600 rounded text-lg ${isWinner ? "text-green-700 dark:text-green-400" : "text-gray-700 dark:text-gray-300"}`}
                        >
                          {qs.final}
                        </td>
                      </tr>
          ...
          {coachingInsights.decidingFactors?.length > 0 && (
            <div className="mt-4 pt-4 border-t border-slate-200 dark:border-gray-700">
              ...
              <ul className="space-y-1">
                {coachingInsights.decidingFactors.map(
          ...
          {coachingInsights.forNextGame.playersToWatch?.length > 0 && (
            <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
                {coachingInsights.forNextGame.playersToWatch.map(
                  (p: any, i: number) => (
                    <div
                      key={i}
                      className="p-3 bg-yellow-50 dark:bg-yellow-900/30 rounded-lg border border-yellow-100 dark:border-yellow-800"
                    >
                      <div className="flex items-center gap-2 mb-2">
                        <span className="text-lg font-bold text-yellow-800 dark:text-yellow-400">
                          #{p.jersey}
                        </span>
      ...
      {coachingInsights?.practicePriorities?.length > 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {coachingInsights.practicePriorities.map(
              (priority: any, i: number) => (
                <div
                  key={i}
                  className="bg-indigo-50 dark:bg-indigo-900/30 rounded-lg p-4 border border-indigo-100 dark:border-indigo-800"
                >
      ...
      {coachingInsights?.exploitableTendencies?.length > 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
          ...
          <div className="space-y-4">
            {coachingInsights.exploitableTendencies.map(
              (tendency: any, i: number) => (
                <div
                  key={i}
                  className="bg-orange-50 dark:bg-orange-900/30 rounded-lg p-4 border border-orange-100 dark:border-orange-800"
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm font-semibold text-orange-800 dark:text-orange-400">
                        Counter: {tendency.counter}
                      </div>
                    </div>
                  </div>
                </div>
            ...
            {coachReport.split("\n").map((line: string, i: number) => {
              if (line.startsWith("# ")) {
                return (
                  <h1
                    key={i}
                    className="text-2xl font-bold text-gray-900 dark:text-white mt-4 mb-2"
                  >
                    {line.replace("# ", "")}
                  </h1>
                );
                ...
                return (
                  <h3
                    key={i}
                    className="text-lg font-medium text-gray-700 dark:text-gray-300 mt-4 mb-1"
                  >
                    {line.replace("### ", "")}
                  </h3>
                );
      ...
      {gameInfo && (
        <div className="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-gray-800 dark:to-gray-800 rounded-xl border border-blue-100 dark:border-gray-700 p-6">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className="text-xs text-gray-500 dark:text-gray-400">
                    {gameInfo.teams.home.jerseyColor}
                  </span>
                </div>
              </div>
            )}
            {gameInfo.teams?.away?.name && (
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  />
                  <span className="text-xs text-gray-500 dark:text-gray-400">
                    {gameInfo.teams.away.jerseyColor}
                  </span>
                </div>
              </div>
            )}
          ...
          {gameInfo.finalScore &&
            gameInfo.finalScore.home != null &&
            gameInfo.finalScore.away != null && (
              <div className="mt-4 pt-4 border-t border-blue-100 dark:border-gray-700">
                ...
                <p className="text-2xl font-bold text-gray-900 dark:text-white">
                  ...
                  {gameInfo.finalScore.home} - {gameInfo.finalScore.away}{" "}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
          ...
          {gameFlow.clutchPerformance && (
            <div className="pt-4 border-t border-gray-100 dark:border-gray-700">
              <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Clutch Performance (Final 2 mins)
              </p>
              <div className="grid grid-cols-2 gap-4">
                {gameFlow.clutchPerformance.finalTwoMinutes?.home && (
                  <div className="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                    ...
                    <p className="text-lg font-bold text-gray-900 dark:text-white">
                      {gameFlow.clutchPerformance.finalTwoMinutes.home.points}{" "}
                      pts
                    </p>
                  </div>
                )}
                {gameFlow.clutchPerformance.finalTwoMinutes?.away && (
                  <div className="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                    ...
                    <p className="text-lg font-bold text-gray-900 dark:text-white">
                      {gameFlow.clutchPerformance.finalTwoMinutes.away.points}{" "}
                      pts
                    </p>
                  </div>
                )}
              ...
              {gameFlow.clutchPerformance.clutchPlayers?.length > 0 && (
                <div className="mt-3">
                  <p className="text-xs text-green-600 dark:text-green-400">
                    Clutch Players:{" "}
                    {gameFlow.clutchPerformance.clutchPlayers.join(", ")}
                  </p>
                </div>
              )}
            </div>
      ...
      {game.detectedTeams && game.detectedTeams.length > 0 && (
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
          ...
          {["home", "away"].map((teamLabel) => {
            const team = game.detectedTeams?.find(
              (t: any) => t.teamLabel === teamLabel,
            );
            const teamPlayers = team?.players || [];
            if (teamPlayers.length === 0) return null;

            const teamName =
              team?.teamName || (teamLabel === "home" ? "Home" : "Away");
            const teamColor =
              team?.primaryJerseyColor ||
              (teamLabel === "home" ? "#0f2d52" : "#dc2626");
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {teamPlayers
                        .sort(
                          (a: any, b: any) =>
                            (b.analysis?.metrics?.points || 0) -
                            (a.analysis?.metrics?.points || 0),
                        )
                        .map((player: any, idx: number) => {
                          ...
                          return (
                            <tr
                              key={idx}
                              className="border-b border-gray-100 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 text-gray-900 dark:text-gray-100"
                            >
...
function ProcessingStatus({ game }: { game: any }) {
  ...
  const isProcessing = ["detecting", "tracking", "analyzing"].includes(
    game.status,
  );
...
export default function GameDetailPage({
  ...
  const [activeTab, setActiveTab] = useState<
    "insights" | "players" | "teams" | "plays" | "video"
  >("insights");
  ...
  const videoMarkers = useMemo(() => {
    ...
    data.game.detectedPlays?.forEach((play: any) => {
      const startTime = play.startTimestamp
        ? parseFloat(play.startTimestamp)
        : play.startTime
          ? play.startTime
          : null;
      if (startTime !== null) {
        markers.push({
          ...
          time: startTime,
          label:
            `Play ${play.playNumber || ""}${play.formation ? ` - ${play.formation}` : ""}${play.playType && play.playType !== "unknown" ? ` (${play.playType})` : ""}`.trim(),
          type: "play",
          color: "#f59e0b",
        });
      }
  ...
  const geminiPlayerMap = useMemo(() => {
    ...
    const players = Array.isArray(playerScouting)
      ? playerScouting
    ...
    <div className="p-6 lg:p-8 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-8">
        ...
        <PlayerDetailModal
          ...
          teamName={
            selectedPlayer?.team?.toLowerCase() === "home"
              ? game.geminiAnalysis?.homeTeamName
              : game.geminiAnalysis?.awayTeamName
          }
        ...
        <div>
                ...
                <>
                      ...
                      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                        {scoutedPlayers
                          .filter(
                            (player: any) =>
                              playerTeamFilter === "all" ||
                              player.team?.toLowerCase() === playerTeamFilter,
                          )
                          .map((player: any, idx: number) => (
                            <ScoutingPlayerCard
                              key={idx}
                  ...
                  {scoutedPlayers.length === 0 && (
                    <>
                      <div className="flex items-center justify-between mb-4">
                        <span className="text-sm text-gray-500 dark:text-gray-400">
                          {allPlayers.length} players detected
                        </span>
                      </div>
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                          {allPlayers.map((player: any) => (
                            <PlayerCard
                              ...
                              geminiPlayerData={geminiPlayerMap.get(
                                player.jerseyNumber,
                              )}
                            />
                          ))}
          ...
          {activeTab === "plays" && (
            <>
              ...
              {game.geminiAnalysis?.coachingInsights?.keyMoments?.length >
                0 && (
                <div className="mb-8">
                  <h3 className="font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                  ...
                  <div className="grid gap-3">
                    {game.geminiAnalysis.coachingInsights.keyMoments.map(
                      (moment: any, idx: number) => {
                        ...
                        return (
                          <div
                            ...
                            onClick={() =>
                              seconds !== null && openVideoAt(seconds)
                            }
                          >
                            <div className="flex items-start justify-between gap-4">
                              ...
                              {game.videoUrl && seconds !== null && (
                                <Button
                                  variant="ghost"
                                  size="sm"
                                  className="gap-1 text-yellow-700 hover:text-yellow-800 shrink-0"
                                >
              ...
              <div className="flex items-center justify-between mb-4">
                <span className="text-sm text-gray-500 dark:text-gray-400">
                  {game.detectedPlays?.length || 0} plays detected
                </span>
                {game.videoUrl && (
                  <Button
                    variant="outline"
                ...
                {game.detectedPlays?.map((play: any, index: number) => {
                  const startTime = play.startTimestamp
                    ...
                    : play.startTime
                      ? play.startTime
                      : null;
                  const endTime = play.endTimestamp
                    ? parseFloat(play.endTimestamp)
                    : play.endTime
                      ? play.endTime
                      : null;
                  ...
                  return (
                    <div
                      ...
                      onClick={() =>
                        startTime !== null && openVideoAt(startTime)
                      }
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
            ...
            <>
              ...
              {!game.geminiAnalysis?.teamScouting && (
                <>
                  <div className="flex items-center justify-between mb-4">
                    <span className="text-sm text-gray-500 dark:text-gray-400">
                      {game.detectedTeams?.length || 0} teams detected
                    </span>
                  </div>
          ...
          {activeTab === "video" &&
            ...
            (() => {
              const keyMoments =
                game.geminiAnalysis?.coachingInsights?.keyMoments || [];
              const scoringRuns =
                game.geminiAnalysis?.coachingInsights?.scoringRuns || [];
