...
function ReportTypeCard({
  ...
  return (
    ...
    >
      <Icon
        className={`w-8 h-8 mb-2 ${selected ? "text-[#0f2d52]" : "text-gray-400"}`}
      />
      <h3
        className={`font-semibold ${selected ? "text-[#0f2d52]" : "text-gray-900 dark:text-white"}`}
      >
        {title}
      </h3>
...
function PlayerReportPreview({ report }: { report: PlayerReport }) {
  ...
  return (
    <div
      className="bg-white dark:bg-gray-800 print:bg-white"
      id="report-content"
    >
      {/* Header */}
      <div className="flex items-start justify-between border-b border-gray-200 dark:border-gray-700 pb-4 mb-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-gray-900 dark:text-white">
              #{report.jerseyNumber}{" "}
              {report.displayName !== `#${report.jerseyNumber}`
                ? report.displayName
                : ""}
            </h2>
        ...
        <div className="text-right">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            {report.gamesPlayed} game{report.gamesPlayed !== 1 ? "s" : ""}{" "}
            analyzed
          </p>
        </div>
      </div>
            ...
            {typeof report.tendencies === "object" ? (
              <ul className="space-y-1">
                {report.tendencies.primaryRole && (
                  <li>
                    • <strong>Role:</strong> {report.tendencies.primaryRole}
                  </li>
                )}
                {report.tendencies.offenseStyle && (
                  <li>
                    • <strong>Offense:</strong> {report.tendencies.offenseStyle}
                  </li>
                )}
                {report.tendencies.defenseStyle && (
                  <li>
                    • <strong>Defense:</strong> {report.tendencies.defenseStyle}
                  </li>
                )}
                {report.tendencies.preferredHand && (
                  <li>
                    • <strong>Preferred hand:</strong>{" "}
                    {report.tendencies.preferredHand}
                  </li>
                )}
                {report.tendencies.preferredDirection && (
                  <li>
                    • <strong>Tends to go:</strong>{" "}
                    {report.tendencies.preferredDirection}
                  </li>
                )}
                {report.tendencies.primaryMoves
                  ?.slice(0, 3)
                ...
                <div className="flex items-center gap-4">
                  {player.stats && (
                    <span className="text-sm text-gray-500 dark:text-gray-400">
                      {player.stats.points || 0}p / {player.stats.rebounds || 0}
                      r / {player.stats.assists || 0}a
                    </span>
                  )}
          ...
          {report.topPlayers.slice(0, 5).map((player, i) => (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className="font-medium text-gray-900 dark:text-white">
                  #{player.jerseyNumber}{" "}
                  {player.name && player.name !== `#${player.jerseyNumber}`
                    ? player.name
                    : ""}
                </span>
              </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col print:border-0 print:shadow-none print:max-w-none print:rounded-none print:max-h-none">
          ...
          <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700 print:hidden flex-shrink-0">
            ...
            <div className="flex items-center gap-2">
              <Button
                onClick={handlePrint}
                className="bg-[#0f2d52] hover:bg-[#1a3d62]"
              >
...
export default function ReportsPage() {
  ...
  const players: PlayerOption[] = (playersData?.players || []).filter(
    (p: PlayerOption) =>
      p.gamesPlayed > 0 && p.avgGrade !== null && p.avgGrade > 0,
  );
  const games: GameOption[] = (gamesData?.games || [])
    .filter((g: any) => g.status === "ready" && g.detectedPlayers?.length > 0)
    .map((g: any) => ({
      id: g.id,
      name: g.title || g.name || "Game",
      date: g.gameDate || g.createdAt,
      playerCount: g.detectedPlayers?.length || 0,
      status: g.status,
    }));
    ...
    <div className="max-w-5xl mx-auto">
      ...
      >
        {reportData?.report ? (
          <>
            {reportType === "player" && (
              <PlayerReportPreview report={reportData.report} />
            )}
            {reportType === "game" && (
              <GameReportPreview report={reportData.report} />
            )}
            {reportType === "season" && (
              <SeasonReportPreview report={reportData.report} />
            )}
          </>
        ) : (
