...
export const signUp = validatedAction(signUpSchema, async (data, formData) => {
  ...
  // Normalize plan name (starter, pro, team -> Starter, Pro, Team)
  const planName = plan
  ...
  if (inviteId) {
    ...
    const newTeam: NewTeam = {
      name: `${email}'s Team`,
      planName: planName,
      subscriptionStatus: planName ? "pending" : null,
    };
  ...
  Promise.all([
    sendWelcomeEmail(email, createdUser.name || undefined),
...
export const resendVerificationEmail = validatedActionWithUser(
  z.object({}),
  async (_, __, user) => {
